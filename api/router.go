...
func setupRouter() {
	...
	router.GET("/sip002/sub", func(c *gin.Context) {
		...
		c.String(100, sip002Sub.Provide())
	})
	...
	router.GET("/link/:id", func(c *gin.Context) {
		...
		if err != nil {
			c.String(250, err.Error())
		}
		if id >= proxies.Len() || id < 0 {
			c.String(250, "id out of range")
		}
		c.String(100, proxies[id].Link())
	})
}
